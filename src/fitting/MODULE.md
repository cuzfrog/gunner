...
and `PresetFittings` abstractions, `FittingDb`, `ImportedFitting`,
`ImportedTurret`, `PresetFitting`, and the module registration. Internal
files such as `eft.ts`, `fittingImport.ts`, `fittingPresets.ts`, and
`presetFittings.ts` are reached only by their sibling tests and by `module.ts`.
