...
Gate note (single-source drains): `capacitorCalculator.resolve` takes a `CapacitorDrainSources` DTO (`defense`, `turrets`, `ewar`, `boosts`, `missileBoosts`, `sensorBoosts`) — the resolved products `FittingImport` already builds — and derives usage rows from them: turret rows from `ImportedTurret` (capacitorNeed/cycleTime/turretCount; the db re-lookup and index pairing into `turretGroups` are gone), ewar/booster/neutralizer rows via the sim's `scheduledDrainsFromProjections` (the exact extraction the runtime uses, grouped by moduleId with instance counts), the RAH row from `DefenseSpec.rah`, and the propulsion interval from the sim's `PROPULSION_CYCLE_SECONDS`. The `findActiveCapFamily` re-derivation and the preview-side `PROPULSION_CYCLE_TIME` constant were deleted, so the static usage table cannot diverge from the runtime drains. `defenseCalculator.resolveRahSpec` now carries `capacitorNeed` from the db stats: previously the preview showed the RAH row but `RahSpec.capacitorNeed` stayed undefined and the sim never debited RAH activation.
