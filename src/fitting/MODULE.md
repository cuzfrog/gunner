...
no-new-exports:
  ...
  # - missileStats.ts (temporarily ungated: missileMagazineShots domain export, see gate note at bottom)
  - droneCatalog.ts
  - droneCatalog.test.ts
  - droneLoadoutResolver.ts
  - droneLoadoutResolver.test.ts
  - droneLoadoutLimits.ts
  - droneLoadoutLimits.test.ts
...
Gate note (ECM jammers): `fittingCalculator.ts` (gated) gained two underscore test-seam exports at the file bottom — `export { resolveSensorStats as _resolveSensorStats }` and `export { resolveEwarState as _resolveEwarState }` — because `resolveEwar` was decomposed into file-bottom functions with narrow params (`resolveEwarState(db: EwarDb, itemNameCatalog, ewarModules, skillLevel)`) to keep the jammers family wired through the same skill/cap scaling as the other ewar families; the underscore exports signal internal seams, not public API. `fittingImport.ts`, `fittingState.ts`, and `capacitorCalculator.ts` changed bodies only: jammers classify as ewar/module roles and join the scheduled capacitor drain families.

Gate note (launcher magazines): `missileStats.ts` was temporarily ungated (frontmatter line commented out) to export `missileMagazineShots(launcher, missile, launcherCount)` — the pure launcher-x-charge magazine computation `floor(floor(capacity / volume) / chargeRate) * count` (pyfa `getNumCharges` + `__calculateAmmoShots`), consumed by the two `ImportedLauncher` producers: `fittingCalculator.resolveLauncher` and `MissileCatalogImpl.withCharge` (the UI ammo switch must re-derive the magazine because charge volume changes with the loaded missile). The db supports it: `LauncherStats` gained `capacity?`/`chargeRate?`/`reloadTime?` (seconds) and `MissileStats` gained `volume?`; `ImportedLauncher` gained `magazineShots?`/`reloadTime?`; the sim `MissileSpec` gained `magazine?` (`numShots`, `reloadTime`) and the missile simulator counts shots down per launch, stretching the final cooldown by the reload time and refilling after.

Gate note (command burst effects): `fittingImport.ts` resolves `ImportedFitting.commandBursts` from the fitting's command burst modules: the charge's warfare buffs map to sim `BurstEffectSpec`s through a file-private `WARFARE_BUFF_KINDS` table (pyfa `__runCommandBoosts` buff ids) and `burstEffects(db, chargeId)`. Interface member addition and private functions only, no new exports.
...
Gate note (drone launch budget): `droneLoadoutLimits.ts` is a new internal file (added to `no-new-exports`) exporting the module-local `droneLoadoutLimits(profile, hullBonuses)` — the single source of the drone limits (`maxActiveDrones`, `bandwidthLimit`, `capacityLimit`, flat hull bonuses included). `droneLoadoutValidator.ts` (gated, body change) consumes it instead of its private `flatSum` derivation; `droneLoadoutResolver.ts` (gated, body change, deps gained `fittingDb: Pick<FittingDb, "combatDrones">`) applies the launch projection before resolving: `launchClampedGroups` walks the bay groups in order and clamps each count by the remaining bandwidth and active-drone slots, skipping groups that cannot launch and dropping unknown type ids, so the resolved `ImportedDrone[]` (the sim-facing specs, session telemetry, and canvas readouts all derive from it) never sends more drones to attack than the drone bandwidth allows. The bay loadout itself is not clamped — `fittingCalculator.resolveDrones` keeps resolving raw bay groups for `ImportedFitting.drones` (bay seeding and import preview), and the validator keeps reporting bay violations for the UI summary. Producer-side coverage: `droneLoadoutResolver.test.ts` clamp suite and `tests/scenarios/drone-launch-budget.test.ts` (real FittingImportImpl against FITTING_DB: Ishtar 125 bandwidth, Ogre II x10 launches 5, Acolyte I x10 launches 5 by maxActiveDrones). No new exports from gated files.
Gate note (drone/fighter bay vs launch split): `DroneGroup`/`FighterGroup` carry `activeCount` (the launched subset of the bay stock, never above `count`) alongside `count`. `droneLoadoutValidator.ts` (gated) flags launch violations (`tooManyDrones`/`bandwidthExceeded`) against the launched set and `bayCapacityExceeded` against the stored volume (`DroneLoadoutValidation` reshaped: `activeCount`/`activeBandwidth` replace `totalBandwidth`); `fighterLoadoutValidator.ts` (gated) computes squadrons from `activeCount` and hangar volume from the total. `fittingCalculator.resolveDrones`/`resolveFighters` project `group.activeCount` (zero-active groups resolve nothing) and the drone resolver clamps `activeCount` by the launch budget. Consumers migrate accordingly: settings v16 (`settingsCompat.migrateDroneActiveCounts`, `validators.ts` optional `activeCount` guard) maps legacy groups to fully launched, profile text parses a missing `activeCount` as `count`, EFT import stays all-launched. Body changes only, no new exports from gated files.
