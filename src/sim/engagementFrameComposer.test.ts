...
describe("EngagementFrameComposerImpl", () => {
  test("composes a view from the engagement evaluator result", () => {
    ...
    expect(engagementEvaluator.evaluate).toHaveBeenCalledWith(frame, {
      shipA: { weapon: shipATurret, opponentSigRadius: 40, droneState: undefined, missileFacts: undefined, spoolCycles: 0, locked: true },
      shipB: { weapon: shipBTurret, opponentSigRadius: 30, droneState: undefined, missileFacts: undefined, spoolCycles: 0, locked: true },
    });
  });
  ...
  test("threads per-weapon spoolCycles into the attack state", () => {
    ...
    expect(vi.mocked(engagementEvaluator.evaluate).mock.calls[0][1].shipA).toEqual({ weapon: shipATurret, opponentSigRadius: 40, droneState: undefined, missileFacts: undefined, spoolCycles: 2, locked: true });
    expect(vi.mocked(engagementEvaluator.evaluate).mock.calls[1][1].shipA).toEqual({ weapon: shipBTurret, opponentSigRadius: 40, droneState: undefined, missileFacts: undefined, spoolCycles: 5, locked: true });
  });
  ...
  test("readouts produce missile values when the effective weapon is a missile", () => {
    ...
    const view = composer.compose(snapshot, { weapons: { shipA: [missile], shipB: [] }, sigRadii: { shipA: 30, shipB: 40 }, droneStates: { shipA: [], shipB: [] }, missileFacts: { shipA: [], shipB: [] }, defenses: { shipA: EMPTY_DEFENSE_SPEC, shipB: EMPTY_DEFENSE_SPEC }, overloaded: { shipA: false, shipB: false }, spoolCycles: { shipA: [], shipB: [] } as const, locks: { shipA: LOCKED_STATE, shipB: LOCKED_STATE } });
  ...
  test("readouts produce drone values when the effective weapon is a drone", () => {
    ...
    const view = composer.compose(snapshot, { weapons: { shipA: [drone], shipB: [] }, sigRadii: { shipA: 30, shipB: 40 }, droneStates: { shipA: [], shipB: [] }, missileFacts: { shipA: [], shipB: [] }, defenses: { shipA: EMPTY_DEFENSE_SPEC, shipB: EMPTY_DEFENSE_SPEC }, overloaded: { shipA: false, shipB: false }, spoolCycles: { shipA: [], shipB: [] } as const, locks: { shipA: LOCKED_STATE, shipB: LOCKED_STATE } });
...
describe("EngagementFrameComposerImpl incomingOffensiveModules", () => {
  test("active weapon from shipA appears under shipB target", () => {
    ...
    const view = composer.compose(snapshot, { weapons: { shipA: [shipATurret], shipB: [] }, sigRadii: { shipA: 30, shipB: 40 }, droneStates: { shipA: [], shipB: [] }, missileFacts: { shipA: [], shipB: [] }, defenses: { shipA: EMPTY_DEFENSE_SPEC, shipB: EMPTY_DEFENSE_SPEC }, overloaded: { shipA: false, shipB: false }, spoolCycles: { shipA: [], shipB: [] } as const, locks: { shipA: LOCKED_STATE, shipB: LOCKED_STATE } });
  ...
  test("active weapon from shipB appears under shipA target", () => {
    ...
    const view = composer.compose(snapshot, { weapons: { shipA: [], shipB: [shipBTurret] }, sigRadii: { shipA: 30, shipB: 40 }, droneStates: { shipA: [], shipB: [] }, missileFacts: { shipA: [], shipB: [] }, defenses: { shipA: EMPTY_DEFENSE_SPEC, shipB: EMPTY_DEFENSE_SPEC }, overloaded: { shipA: false, shipB: false }, spoolCycles: { shipA: [], shipB: [] } as const, locks: { shipA: LOCKED_STATE, shipB: LOCKED_STATE } });
