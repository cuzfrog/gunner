...
describe("SimulationImpl", () => {
  ...
  test("an active scrambler reverts MWD sig bloom to baseSig", () => {
    const resolver: EwarResolver = {
      speedMultiplier: () => 1,
      speedMultiplierIgnoringRange: () => 1, sigMultiplier: () => 1, sigMultiplierIgnoringRange: () => 1, disruptedTurret: (turret) => turret,
      disruptedTurretIgnoringRange: (turret) => turret,
    ...
    const config: SimConfig = {
      shipA: shipConfig("shipA", "midships"),
      shipB: { ...shipConfig("shipB", "midships", 1, 1e-6, 200, 200), maxSpeed: 1800, sig: 1200, baseSig: 200 },
      initialDistance: 5000,
    };
    const sim = new SimulationImpl({ shipASteering: steering, shipBSteering: steering, ewarResolver: resolver, simConfig: config });
    sim.step(1);
  ...
  test("without scrambler suppression, MWD sig bloom stays active", () => {
    const resolver: EwarResolver = {
      speedMultiplier: () => 1,
      speedMultiplierIgnoringRange: () => 1, sigMultiplier: () => 1, sigMultiplierIgnoringRange: () => 1, disruptedTurret: (turret) => turret,
      disruptedTurretIgnoringRange: (turret) => turret,
    ...
    const sim = new SimulationImpl({ shipASteering: steering, shipBSteering: steering, ewarResolver: resolver, simConfig: config });
    sim.step(1);
    expect(sim.snapshot().shipB.sig).toBeCloseTo(1200, 6);
  });

  test("an active scrambler leaves sig unchanged when baseSig is absent", () => {
    const resolver: EwarResolver = {
      speedMultiplier: () => 1,
      speedMultiplierIgnoringRange: () => 1, sigMultiplier: () => 1, sigMultiplierIgnoringRange: () => 1, disruptedTurret: (turret) => turret,
      disruptedTurretIgnoringRange: (turret) => turret,
    ...
    const config: SimConfig = {
      shipA: shipConfig("shipA", "midships"),
      shipB: { ...shipConfig("shipB", "midships", 1, 1e-6, 200, 200), maxSpeed: 1800, sig: 1200 },
      initialDistance: 5000,
    };
    const sim = new SimulationImpl({ shipASteering: steering, shipBSteering: steering, ewarResolver: resolver, simConfig: config });
    sim.step(1);
