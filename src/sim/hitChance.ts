...
export interface HitChance {
  compute(frame: EngagementFrame, turret: TurretSpec, shipBSigRadius: number): HitChanceBreakdown;
  findBestDistance(transversalSpeed: number, turret: TurretSpec, shipBSigRadius: number): number;
}
...
export class HitChanceImpl implements HitChance {
  compute(frame: EngagementFrame, turret: TurretSpec, shipBSigRadius: number): HitChanceBreakdown {
    ...
    let trackingTerm = 0;
    if (tracking > 0 && shipBSigRadius > 0) {
      trackingTerm = ((angularVelocity * sigResolution) / (tracking * shipBSigRadius)) ** 2;
  ...
  /**
   * Find the distance that maximizes hit chance for a shipB orbiting at
   ...
   * where A = (transversalSpeed * sigRes) / (tracking * shipBSig).
   */
  findBestDistance(transversalSpeed: number, turret: TurretSpec, shipBSigRadius: number): number {
    const { tracking, sigResolution, optimal, falloff } = turret;

    if (transversalSpeed <= 0 || tracking <= 0 || shipBSigRadius <= 0 || falloff <= 0) {
      return optimal;
    }

    const a = (transversalSpeed * sigResolution) / (tracking * shipBSigRadius);
