...
function stepBoth(shipA: ShipState, shipB: ShipState, dt: number): void {
  const shipAVel = command(shipA, shipB);
  const shipBVel = command(shipB, shipA);
  shipA.velocity = shipAVel;
  shipB.velocity = shipBVel;
  shipA.position = shipA.position.add(shipAVel.scale(dt));
  shipB.position = shipB.position.add(shipBVel.scale(dt));
}

function runBoth(shipA: ShipState, shipB: ShipState, dt: number, steps: number): number {
  for (let i = 0; i < steps; i++) {
    stepBoth(shipA, shipB, dt);
  }
...
describe("ReactiveAutopilot", () => {
  test("orbit points tangentially around the reference", () => {
    const ship = makeShip("shipB", [0, 5000], "orbit", 1000, 5000);
    const other = makeShip("shipA", [0, 0], "orbit", 0, 5000);
  ...
  describe("keepAtRange", () => {
    test("slower keeper yields to a faster shipB's desired range", () => {
      const shipA = makeShip("shipA", [0, 0], "keepAtRange", 1300, 10000);
      const shipB = makeShip("shipB", [0, 12000], "keepAtRange", 1500, 15000);
      const finalDistance = runBoth(shipA, shipB, DT, STEPS);
      expect(finalDistance).toBeGreaterThan(14500);
      expect(finalDistance).toBeLessThan(15500);
    });

    test("faster shipA holds its own desired range", () => {
      const shipA = makeShip("shipA", [0, 0], "keepAtRange", 1500, 10000);
      const shipB = makeShip("shipB", [0, 12000], "keepAtRange", 1300, 15000);
      const finalDistance = runBoth(shipA, shipB, DT, STEPS);
    ...
    test("keeper holds its setpoint against a faster closing ship", () => {
      const shipA = makeShip("shipA", [0, 0], "keepAtRange", 1500, 15000);
      const shipB = makeShip("shipB", [0, 12000], "keepAtRange", 1300, 5000);
      const finalDistance = runBoth(shipA, shipB, DT, STEPS);
      expect(finalDistance).toBeGreaterThan(14500);
      expect(finalDistance).toBeLessThan(15500);
    });

    test("keeper closes on an outward-flying shipB at the speed difference", () => {
      const shipA = makeShip("shipA", [0, 0], "keepAtRange", 1500, 15000);
      const shipB = makeShip("shipB", [0, 50000], "keepAtRange", 800, 100000);
      const finalDistance = runBoth(shipA, shipB, DT, STEPS);
      expect(finalDistance).toBeGreaterThan(14500);
      expect(finalDistance).toBeLessThan(15500);
    });
    ...
    test("slower keeper aims at its own range against a faster keeper", () => {
      const ship = makeShip("shipA", [0, 0], "keepAtRange", 1000, 10000);
      const fasterKeeper = makeShip("shipB", [0, 15000], "keepAtRange", 1200, 15000);
    ...
    test("feedforward cancels opponent radial velocity", () => {
      const ship = makeShip("shipA", [0, 0], "keepAtRange", 1500, 15000);
      const approacher = makeShip("shipB", [0, 15000], "keepAtRange", 1300, 5000);
      approacher.velocity = new Vec2(0, -1300);
      expect(command(ship, approacher).y).toBeCloseTo(-1300, 5);

      const closer = makeShip("shipB", [0, 12000], "keepAtRange", 1300, 5000);
      closer.velocity = new Vec2(0, -1300);
      expect(command(ship, closer).y).toBeCloseTo(-1500, 5);

      const retreater = makeShip("shipB", [0, 15000], "keepAtRange", 800, 5000);
    ...
    test("braking reduces the closing command for a ship with real dynamics", () => {
      const ship = makeShip("shipA", [0, 0], "keepAtRange", 1000, 1000, 2_000_000, 1, 1);
      const other = makeShip("shipB", [0, 1200], "keepAtRange", 0, 1000);
      ship.velocity = new Vec2(0, 300);

      expect(command(ship, other).y).toBeCloseTo(100, 5);
      expect(command(makeShip("shipA", [0, 0], "keepAtRange", 1000, 1000, 2_000_000, 1, 100), other).y).toBeCloseTo(400, 5);
    });

    test("instant-dynamics ships are unchanged by braking", () => {
      const ship = makeShip("shipA", [0, 0], "keepAtRange", 1000, 1000, 1, 1e-6, 0.01);
      const other = makeShip("shipB", [0, 1200], "keepAtRange", 0, 1000);
      ship.velocity = new Vec2(0, 300);

      expect(command(ship, other).y).toBeCloseTo(400, 5);
      expect(command(makeShip("shipA", [0, 0], "keepAtRange", 1000, 1000, 1, 1e-6, 100), other).y).toBeCloseTo(400, 5);
    });

    test("keep-at-range damping is monotonic across the aggressivity bar", () => {
      const other = makeShip("shipB", [0, 1200], "keepAtRange", 0, 1000);
      const make = (aggressivity: number) =>
        makeShip("shipA", [0, 0], "keepAtRange", 1000, 1000, 2_000_000, 1, aggressivity);
  ...
  describe("orbit", () => {
    test("slower keeper yields to a faster orbit's desired range", () => {
      const shipA = makeShip("shipA", [0, 0], "keepAtRange", 1300, 10000);
      const shipB = makeShip("shipB", [0, 12000], "orbit", 1500, 15000);
      const finalDistance = runBoth(shipA, shipB, DT, STEPS);
      expect(finalDistance).toBeGreaterThan(14500);
      expect(finalDistance).toBeLessThan(15500);
    });
    ...
    test("faster orbit holds its own desired range against a slower keeper", () => {
      const shipA = makeShip("shipA", [0, 0], "orbit", 1500, 10000);
      const shipB = makeShip("shipB", [0, 12000], "keepAtRange", 1300, 15000);
      const finalDistance = runBoth(shipA, shipB, DT, STEPS);
      expect(finalDistance).toBeGreaterThan(9500);
      expect(finalDistance).toBeLessThan(10500);
    ...
    test("equal-speed closing keeper does not tackle orbit", () => {
      const shipA = makeShip("shipA", [0, 0], "keepAtRange", 1300, 5000);
      const shipB = makeShip("shipB", [0, 12000], "orbit", 1300, 12000);
      const finalDistance = runBoth(shipA, shipB, DT, STEPS);
      expect(finalDistance).toBeGreaterThan(11500);
      expect(finalDistance).toBeLessThan(12500);
    });
    ...
    test("faster orbit holds its radius against an outward-flying ship", () => {
      const shipA = makeShip("shipA", [0, 0], "orbit", 1500, 10000);
      const shipB = makeShip("shipB", [0, 10000], "keepAtRange", 800, 50000);
      const finalDistance = runBoth(shipA, shipB, DT, STEPS);
      expect(finalDistance).toBeGreaterThan(9500);
      expect(finalDistance).toBeLessThan(10500);
    });
    ...
    test("faster orbit wins range contest against slower orbit", () => {
      const shipA = makeShip("shipA", [0, 0], "orbit", 1500, 10000);
      const shipB = makeShip("shipB", [0, 12000], "orbit", 1300, 15000);
      const finalDistance = runBoth(shipA, shipB, DT, STEPS);
      expect(finalDistance).toBeGreaterThan(9500);
      expect(finalDistance).toBeLessThan(10500);
    });
    ...
    test("orbit tangential budget shrinks as radial demand grows", () => {
      const nearShip = makeShip("shipA", [0, 0], "orbit", 1000, 10000);
      const nearOther = makeShip("shipB", [0, 12000], "orbit", 1200, 15000);
      const nearVel = command(nearShip, nearOther);
      expect(nearVel.len()).toBeCloseTo(1000, 5);

      const farShip = makeShip("shipA", [0, 0], "orbit", 1000, 10000);
      const farOther = makeShip("shipB", [0, 30000], "orbit", 800, 5000);
    ...
    test("feeds forward inward lead against dynamics lag", () => {
      const ship = makeShip("shipB", [14000, 0], "orbit", 1500, 14000, 10_000_000, 0.45, 1, "cw");
      const other = makeShip("shipA", [0, 0], "orbit", 0, 5000);
    ...
    test("lag compensation is inward for both orbit directions", () => {
      const ship = makeShip("shipB", [14000, 0], "orbit", 1500, 14000, 10_000_000, 0.45, 1, "ccw");
      const other = makeShip("shipA", [0, 0], "orbit", 0, 5000);
      const fromCenterHat = new Vec2(1, 0);
    ...
    test("no lead with instant dynamics", () => {
      const ship = makeShip("shipB", [14000, 0], "orbit", 1500, 14000);
      const other = makeShip("shipA", [0, 0], "orbit", 0, 5000);
      const fromCenterHat = new Vec2(1, 0);
      const tHat = fromCenterHat.perpCW();
      ship.velocity = tHat.scale(1400);
    ...
    test("lead saturates at the speed budget", () => {
      const ship = makeShip("shipB", [1000, 0], "orbit", 1500, 1000, 10_000_000, 0.45, 1, "cw");
      const other = makeShip("shipA", [0, 0], "orbit", 0, 5000);
      const fromCenterHat = new Vec2(1, 0);
      const tHat = fromCenterHat.perpCW();
    ...
    test("holds its range around a tangentially fleeing center", () => {
      const shipA = makeShip("shipA", [0, 0], "orbit", 1000, 5000, 1, 1e-6, 1, "cw");
      const shipB = makeShip("shipB", [0, 5000], "orbit", 1000, 5000, 1, 1e-6, 1, "ccw");
      const finalDistance = runBoth(shipA, shipB, DT, STEPS);
    ...
    test("braking reduces the inward radial command when closing on the orbit radius", () => {
      const ship = makeShip("shipB", [0, 12000], "orbit", 1000, 10000, 2_000_000, 1, 1);
      const other = makeShip("shipA", [0, 0], "orbit", 0, 5000);
      const fromCenterHat = ship.position.sub(other.position).scale(1 / ship.position.dist(other.position));
      ...
      const undamped = makeShip("shipB", [0, 12000], "orbit", 1000, 10000, 2_000_000, 1, 100);
    ...
    test("braking vanishes for purely tangential and stationary states", () => {
      ...
      const aggressiveTangential = makeShip("shipB", [0, 12000], "orbit", 1000, 10000, 2_000_000, 1, 100);
      ...
      const strictStationary = makeShip("shipB", [0, 12000], "orbit", 1000, 10000, 2_000_000, 1, 0.01);
    ...
    test("aggressivity below the minimum clamps to critical damping", () => {
      ...
      const shipLow = makeShip("shipB", [0, 12000], "orbit", 1000, 10000, 2_000_000, 1, 0.001);
      const shipMin = makeShip("shipB", [0, 12000], "orbit", 1000, 10000, 2_000_000, 1, 0.01);
      const shipHigh = makeShip("shipB", [0, 12000], "orbit", 1000, 10000, 2_000_000, 1, 100);
      const other = makeShip("shipA", [0, 0], "orbit", 0, 5000);
  ...
  describe("midships", () => {
    test("moving ship burns straight ahead at max speed", () => {
      const ship = makeShip("shipA", [0, 0], "midships", 1000);
      ship.velocity = new Vec2(300, 400);
      const other = makeShip("shipB", [0, 5000], "orbit", 0, 5000);
      const cmd = command(ship, other);
    ...
    test("at rest it honors the orbit direction handedness", () => {
      const ship = makeShip("shipA", [0, 0], "midships", 1000, 5000, 1, 1e-6, 1, "ccw");
      const other = makeShip("shipB", [0, 5000], "orbit", 0, 5000);
      const cmd = command(ship, other);
      expect(cmd.x).toBeCloseTo(-1000, 5);
      expect(cmd.y).toBeCloseTo(0, 5);
    ...
    test("stationary and co-located with the other ship returns zero", () => {
      const ship = makeShip("shipA", [0, 0], "midships", 1000);
      const other = makeShip("shipB", [0, 0], "orbit", 0, 5000);
      const cmd = command(ship, other);
      expect(cmd.x).toBe(0);
      expect(cmd.y).toBe(0);
    });
