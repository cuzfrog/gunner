...
class FakeElement {
  ...
  addEventListener(event: string, handler: () => void): void { this.handlers[event] ??= []; this.handlers[event].push(handler); }
  trigger(event: string): void { this.handlers[event]?.forEach((handler) => handler()); }
  setBoundingClientRect(rect: Rect): void { this.rect = rect; }
...
function createDocument(): Document {
  return { createElement: (tagName: string) => { const el = new FakeElement(); el.tagName = tagName; return el as unknown as HTMLElement; } } as unknown as Document;
}
function createI18n(): I18n {
  return { current: () => "en", setLanguage: () => {}, t: (key: string) => key, translateDocument: () => {} };
}
...
const SUMMARY: FittingSummary = {
  ...
  sections: [
    ...
    { kind: "low", rows: [{ name: "400mm Steel Plates II" }] },
    { kind: "cargo", rows: [{ name: "Hail S", quantity: 1000 }] },
  ],
};
describe("DomFittingPreview rendering", () => {
  beforeEach(() => { globalThis.document = createDocument() as unknown as Document; });
  afterEach(() => { globalThis.document = undefined as unknown as Document; });
  ...
  test("drone rows use the actual drone icon when available", () => {
    const { container, anchor, preview } = buildPreview();
    const summary: FittingSummary = { hullName: "Rifter", fittingName: "Brawler", sections: [{ kind: "drones", rows: [{ name: "Hobgoblin II", quantity: 3 }] }] };
    preview.show(anchor as unknown as HTMLElement, summary);
  ...
  test("drone rows fall back to the generic drone icon for unknown drones", () => {
    const { container, anchor, preview } = buildPreview();
    const summary: FittingSummary = { hullName: "Rifter", fittingName: "Brawler", sections: [{ kind: "drones", rows: [{ name: "Unknown Drone", quantity: 3 }] }] };
    preview.show(anchor as unknown as HTMLElement, summary);
    const droneRow = container.children[1].children[1];
    expect(droneRow.children[0].tagName).toBe("img");
