...
export function registerEwarModule<T extends ControlsCradle>(cradle: AwilixContainer<T>): void {
  cradle.register({
    ewarController: asFunction(({ els, popupGroup, imageCatalog, fittingImport, i18n }) => {
      return new EwarControllerImpl({ els: collectEwarEls(els), popupGroup, imageCatalog, fittingImport, i18n });
    }).singleton(),
  });
}
