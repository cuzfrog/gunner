...
Gate note (vorton projectors): `simConfigSource.ts` (gated, body-only) joins the side panel's `importedVortons()` into the engine weapon list through a file-private `toVortonSpec` mapper (the imported vorton is already fully resolved; the spec mirrors it in the sim's weapon shape), and the file-private `SidePanelConfigSource` gains the `importedVortons()` member implemented by `SidePanelImpl`.
Gate note (attack-drones toggle relocation): `simConfigSource.ts` (gated) now reads the tactic from `TargetingController.attackDrones(side)` (new dep wired in `module.ts`) instead of the side panel capture; the toggle lives in the targeting popup owned by the targeting module, so `SidePanelState` no longer carries `attackDrones`. Body changes only, no new exports.
