...
The provider reads the current `EngineView` from `ViewStream`
to obtain the attacker's `AttackAssessment.damage.appliedDps` and the
opponent's `InflictedDps` from `EngineView.inflicted[opponent]`
(`total` and `byLayer`). The opponent is the opposite side:
shipA's inflicted DPS uses shipB's inflicted meter, and vice versa. Layers
