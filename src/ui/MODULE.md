...
Gate relaxed: `renderer.ts` and `index.ts` were removed from `no-new-exports` to replace `TurretRange` with `OptimalFalloffRange` (kind: `"turret" | "drone"`) alongside the existing `MissileRange` union member. Drones share the optimal/falloff ring model with turrets, so a single `OptimalFalloffRange` type encodes both weapon kinds without duplicating the structure.
